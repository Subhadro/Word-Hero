class Spawner {
    ...
    spawnWord() {
        ...
        const pool = this.config.words.filter(w => isPositive ? w.type === 'positive' : w.type === 'negative');
        const wordObj = pool[Math.floor(Math.random() * pool.length)];

        const laneIdx = LaneSystem.getRandomLaneIndex();
        LaneSystem.spawnNode(wordObj, laneIdx, this.config.speed, this.onNodeComplete);
    }
